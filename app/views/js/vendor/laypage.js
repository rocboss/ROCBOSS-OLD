...
function laypage(options){
    var skin = 'laypagecss';
    laypage.dir = 'dir' in laypage ? laypage.dir : Page.getpath + '../css/laypage.css';
    new Page(options);
...
Page.prototype.view = function(){
    ...
    conf.href = 'href' in conf ? conf.href : '#';
    ...
    if(conf.curr > 1 && conf.prev){
        view.push('<a href="'+conf.href.replace('(?)', (conf.curr - 1))+'" onclick="javascript:;return false;" class="laypage_prev" data-page="'+ (conf.curr - 1) +'">'+ conf.prev +'</a>');
    }
    
    //当前组非首组，则输出首页
    if(dict.index > 1 && conf.first && conf.groups !== 0){
        view.push('<a href="'+conf.href.replace('(?)', 1)+'" onclick="javascript:;return false;" class="laypage_first" data-page="1"  title="&#x9996;&#x9875;">'+ conf.first +'</a><span>&#x2026;</span>');
    }
    ...
    for(; dict.start <= dict.end; dict.start++){
        if(dict.start === conf.curr){
            ...
            view.push('<a href="'+conf.href.replace('(?)', dict.start)+'" onclick="javascript:;return false;" data-page="'+ dict.start +'">'+ dict.start +'</a>');
        }
    }
    
    //总页数大于连续分页数，且当前组最大页小于总页，输出尾页
    if(conf.pages > conf.groups && dict.end < conf.pages && conf.last && conf.groups !== 0){
         view.push('<span>&#x2026;</span><a href="'+conf.href.replace('(?)', conf.pages)+'" onclick="javascript:;return false;" class="laypage_last" title="&#x5C3E;&#x9875;"  data-page="'+ conf.pages +'">'+ conf.last +'</a>');
    }
    ...
    if(conf.curr !== conf.pages && conf.next || dict.flow){
        view.push((function(){
            ...
            : '<a href="'+conf.href.replace('(?)', (conf.curr + 1))+'" onclick="javascript:;return false;" class="laypage_next" data-page="'+ (conf.curr + 1) +'">'+ conf.next +'</a>';
        }()));
    }

...
_e(function (E, $) {
    ...
    function loadHljs() {
        ...
        var script = document.createElement("script");
        script.type = "text/javascript";
        script.src = "/app/views/js/vendor/highlight.min.js";
        document.body.appendChild(script);
    }
...
(function (window, E, $) {
    ...
    E.ready(function () {
        ...
        upfile.onSuccess = function (result) {
            ...
            img.onload = function () {
                var html = '<img src="' + result.data + '" style="max-width:100%"/>';
                editor.command(event, 'insertHtml', html);
                img = null;
            };
            img.onerror = function () {
                E.error('使用返回的结果获取图片，发生错误。请确认以下结果是否正确：' + result.data);
                img = null;
            };
            img.src = result.data;
        };
...
(function (window, E, $) {
    ...
    UploadFile.fn.upload = function (file) {
        ...
        reader.onload = function (e) {
            ...
            formData.append('file', UploadFile.convertBase64UrlToBlob(base64, fileType));
            ...
            var onProgressHandler = function(event) {
                msg = layer.msg('图片上传中...', {icon: 16, time: 300000});
                if(event.lengthComputable) {
                    var howmuch = (event.loaded / event.total) * 100;
                    if (Math.ceil(howmuch) == 100) {
                        setTimeout(function() {
                            layer.close(msg)
                        }, 1000);
                    };
